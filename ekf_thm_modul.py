...
def ekf_x_thm_fn(A,ekf_x_duz,t):
    ivme_durumu = baslangic_grafik.ivme_durum_fn(t)[0]
    ekf_x_thm =  np.matmul(A, ekf_x_duz) #ivmesiz
    #ivmeli
    if ivme_durumu !=0:
        ivme_vektor = baslangic_grafik.ivme_durum_fn(t)[1]
        ekf_x_thm = ekf_x_thm + ivme_vektor
    return ekf_x_thm

def ekf_p_thm_fn(A,ekf_p_duz,q_p_matris,delta_t,t):
    ...
    ekf_p_thm = np.matmul(np.matmul(A, ekf_p_duz), A.transpose())
    ekf_p_thm = ekf_p_thm + np.matmul(np.matmul(ro, q_p_matris), ro.transpose())    
